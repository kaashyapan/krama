...
let tests =
  ...
  |> List.map (fun path ->
    ...
    let expected =
      ...
      | "int8" ->
        seq { T.Alias("Test.Int8.Var1", T.Int8) }
        |> Seq.map (fun e -> Seq.contains e types)
      | "uint64" ->
        seq {
      ...
      | "uint8" ->
        seq { T.Alias("Test.Uint8.Var1", T.UInt8) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "result" ->
        seq { T.Alias("Test.Result.Var1", T.Result [ T.String; T.Int32 ]) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "option" ->
        seq { T.Alias("Test.Option.Var1", T.Option T.String) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "list" ->
        seq { T.Alias("Test.List.Var1", T.List T.String) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "array" ->
        seq { T.Alias("Test.Array.Var1", T.Array T.String) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "seq" ->
        seq { T.Alias("Test.Seq.Var1", T.Seq T.String) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "set" ->
        seq { T.Alias("Test.Set.Var1", T.Set T.String) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "voption" ->
        seq { T.Alias("Test.Voption.Var1", T.VOption T.String) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "tuple" ->
        seq { T.Alias("Test.Tuple.Var1", T.Tuple [ T.String; T.Int32 ]) }
        |> Seq.map (fun e -> Seq.contains e types)
      ...
      | "guid" ->
        seq { T.Alias("Test.Guid.Var1", T.Guid) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "union_simple" ->
        seq {
          T.Alias(
            "Test.UnionSimple.Sex",
      ...
      | "anon_record" ->
        seq {
          T.Alias(
            "Test.AnonRecord.Var1",
            T.Option(T.AnonRecord [ T.RecordMember("Name", T.String) ])
          )
        }
        |> Seq.map (fun e -> Seq.contains e types)
      ...
      | "dep_union" ->
        seq {
          T.Alias(
            "Test.DepUnion.Person",
            T.Record [ T.RecordMember("Gender", T.Userdef "Test.DepUnion.Sex") ]
          )
      ...
      | "dep_record" ->
        seq {
          ...
          T.Alias(
            "Test.DepRecord.Var2",
            T.Record [ T.RecordMember("Person", T.Userdef "Test.DepRecord.Var1") ]
          )
        }
        |> Seq.map (fun e -> Seq.contains e types)
      ...
      | "half" ->
        seq { T.Alias("Test.Half.Var1", T.Half) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "double" ->
        seq { T.Alias("Test.Double.Var1", T.Double) }
        |> Seq.map (fun e -> Seq.contains e types)
      ...
      | "dateonly" ->
        seq { T.Alias("Test.DateOnly.Var1", T.DateOnly) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "datetime" ->
        seq { T.Alias("Test.DateTime.Var1", T.DateTime) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "datetimeoffset" ->
        seq { T.Alias("Test.DateTimeOffset.Var1", T.DateTimeOffset) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "timeonly" ->
        seq { T.Alias("Test.TimeOnly.Var1", T.TimeOnly) }
        |> Seq.map (fun e -> Seq.contains e types)

      | "timespan" ->
        seq { T.Alias("Test.TimeSpan.Var1", T.TimeSpan) }
        |> Seq.map (fun e -> Seq.contains e types)
