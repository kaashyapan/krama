...
type WriterModel =
  {
    ...
    /// TypeNotSupported = "fake" writer used in `autoNln`, `autoNlnByFuture`
    Mode: WriteModelMode
    /// current length of last line of output
    Column: int
  }

  member __.IsTypeNotSupported =
    match __.Mode with
...
module WriterModel =
  ...
  let update maxPageWidth cmd m =
    ...
    let updateCmd cmd =
      ...
      | WriteLineInsideStringConst -> { m with Lines = String.empty :: m.Lines; Column = 0 }
      | WriteLineInsideTrivia ->
        let lines =
          match m.Lines with
          | [] -> [ String.empty ]
      ...
      | IndentBy x ->
        { m with Indent = if m.AtColumn >= m.Indent + x then m.AtColumn + x else m.Indent + x }
...
type Context =
  ...
  static member Create hasSource config : Context =
    { Context.Default with Config = config; HasSource = hasSource }

  member x.WithTypeNotSupported(writerCommands, ?keepPageWidth) =
    let keepPageWidth = keepPageWidth |> Option.defaultValue false

    let mkModel m =
      { m with
          Mode = TypeNotSupported
...
let hasWriteBeforeNewlineContent ctx = String.isNotNullOrEmpty ctx.WriterModel.WriteBeforeNewline
...
let dump (isSelection: bool) (ctx: Context) =
  ...
  { Code = code; Cursor = ctx.FormattedCursor }
...
let lastWriteEventOnLastLine ctx = writeEventsOnLastLine ctx |> Seq.tryHead
...
let unindent (ctx: Context) = writerEvent (UnIndentBy ctx.Config.IndentSize) ctx
...
let atIndentLevel alsoSetIndent level (f: Context -> Context) (ctx: Context) =
  if level < 0 then invalidArg "level" "The indent level cannot be negative."
...
let colPost f2 f1 (c: seq<'T>) f (ctx: Context) = if Seq.isEmpty c then ctx else f2 (col f1 c f ctx)

/// Similar to col, apply one more function f2 at the beginning if the input sequence is not empty
let colPre f2 f1 (c: seq<'T>) f (ctx: Context) = if Seq.isEmpty c then ctx else col f1 c f (f2 ctx)
...
let getRecordSize ctx fields =
  ...
  | MultilineFormatterType.NumberOfItems ->
    Size.NumberOfItems(List.length fields, ctx.Config.MaxRecordNumberOfItems)
...
let whenShortIndent f ctx = onlyIf (ctx.Config.IndentSize < 3) f ctx

/// Repeat application of a function n times
let rep n (f: Context -> Context) (ctx: Context) = [ 1..n ] |> List.fold (fun c _ -> f c) ctx
...
let sepSpace (ctx: Context) =
  if ctx.WriterModel.IsTypeNotSupported then
    (!- " ") ctx
  else
...
let addSpaceIfSpaceAfterComma (ctx: Context) = onlyIf ctx.Config.SpaceAfterComma sepSpace ctx
...
let shortExpressionWithFallback
  ...
  | ShortExpression infos when
    (List.exists
      (fun info -> info.ConfirmedMultiline || info.IsTooLong ctx.Config.MaxLineLength ctx.Column)
      infos)
    ->
    ctx
  | _ ->
    ...
    | ShortExpression infos ->
      ...
      then
        ...
        { resultContext with
            WriterModel = { resultContext.WriterModel with Mode = ctx.WriterModel.Mode }
        }
    | _ ->
...
let leadingExpressionResult leadingExpression continuationExpression (ctx: Context) =
  ...
  continuationExpression
    ((lineCountBefore, columnBefore), (lineCountAfter, columnAfter))
    contextAfterLeading
...
let sepSpaceOrIndentAndNlnIfExceedsPageWidthUnlessStroustrup
  isStroustrup
  f
  (node: Node)
  (ctx: Context)
...
let futureNlnCheckMem (f, ctx) =
  if ctx.WriterModel.IsTypeNotSupported then
    ...
    // Create a TypeNotSupported context to evaluate length of current operation
    let TypeNotSupportedCtx: Context =
      ctx.WithTypeNotSupported(Queue.empty, keepPageWidth = true) |> f

    WriterEvents.isMultiline TypeNotSupportedCtx.WriterEvents,
    TypeNotSupportedCtx.Column > ctx.Config.MaxLineLength
...
let exceedsWidth maxWidth f (ctx: Context) =
  let TypeNotSupportedCtx: Context = ctx.WithTypeNotSupported(Queue.empty, keepPageWidth = true)

  let currentLines = TypeNotSupportedCtx.WriterModel.Lines.Length
  let currentColumn = TypeNotSupportedCtx.Column
  let ctxAfter: Context = f TypeNotSupportedCtx
...
let sepComma (ctx: Context) = if ctx.Config.SpaceAfterComma then !- ", " ctx else !- "," ctx
...
let sepNlnWhenWriteBeforeNewlineNotEmptyOr fallback (ctx: Context) =
  if hasWriteBeforeNewlineContent ctx then sepNln ctx else fallback ctx

let sepNlnWhenWriteBeforeNewlineNotEmpty = sepNlnWhenWriteBeforeNewlineNotEmptyOr sepNone

let sepSpaceUnlessWriteBeforeNewlineNotEmpty (ctx: Context) =
  if hasWriteBeforeNewlineContent ctx then ctx else sepSpace ctx

let autoIndentAndNlnWhenWriteBeforeNewlineNotEmpty (f: Context -> Context) (ctx: Context) =
  if hasWriteBeforeNewlineContent ctx then indentSepNlnUnindent f ctx else f ctx
...
let indentSepNlnUnindentUnlessStroustrup f (e: Expr) (ctx: Context) =
  ...
  if shouldUseStroustrup then f e ctx else indentSepNlnUnindent (f e) ctx
...
let autoIndentAndNlnTypeUnlessStroustrup f (t: Type) (ctx: Context) =
  ...
  if shouldUseStroustrup then f t ctx else autoIndentAndNlnIfExpressionExceedsPageWidth (f t) ctx

let autoIndentAndNlnIfExpressionExceedsPageWidthUnlessStroustrup f (e: Expr) (ctx: Context) =
  let isStroustrup = isStroustrupStyleExpr ctx.Config e && canSafelyUseStroustrup (Expr.Node e) ctx

  if isStroustrup then f e ctx else autoIndentAndNlnIfExpressionExceedsPageWidth (f e) ctx
...
type ColMultilineItemsState = { LastBlockMultiline: bool; Context: Context }
...
let colWithNlnWhenItemIsMultiline (items: ColMultilineItem list) (ctx: Context) : Context =
  ...
  | ColMultilineItem(initialExpr, _) :: items ->
    let result =
      ...
      let itemsState = { Context = initialCtx; LastBlockMultiline = initialIsMultiline }
      ...
      let rec loop (acc: ColMultilineItemsState) (items: ColMultilineItem list) =
        ...
        | ColMultilineItem(expr, sepNlnItem) :: rest ->
          ...
          loop { acc with Context = nextCtx; LastBlockMultiline = isMultiline } rest
